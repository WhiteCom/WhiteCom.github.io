...
Single 객체 Lazy Initialization 샘플코드를 살펴보면 아래와 같다. 
...
물론 Lazy initialization 방식으로 사용했을때도 단점은 존재한다. 
여전히 전역객체이기 때문에 호출한 이후 프로그램 종료시까지 계속 메모리에 할당되어있는건 이전과 다른점이 없다. 

그리고 게임같은 경우, Lazy initialization 를 사용하는 경우, 아래와 같은 상황에선 부적합하다. 
...
만약 처음 소리를 재생할 때 Lazy initialization 하게 되면, 전투 도중에 초기화 될 수 있으며 이런 경우, 화면 프레임이 떨어지고 게임경험에 악영향을 준다. 
...
위 문제로 인해, Lazy initialization 를 사용하는 것이 무조건 좋은것은 아니다. 
